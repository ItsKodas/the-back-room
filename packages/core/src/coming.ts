 ...
 * Baccarat is a house game, so it cannot be built without that argument being
 * made again.
 *
 * Poker, Death Rolling, Two-up, Craps and Liar's Dice were on this list and
 * have been built. Two-up had to make the bank argument twice over — once for
 * a casino school paying from its own bank, and once for a traditional school
 * that needs no bank because the chips never leave the ring. Liar's Dice
