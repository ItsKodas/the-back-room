...
export class Table {
  ...
  //
  //   accountOf(seatId): string | null, backed by a private
  //   `accounts = new Map<string, string | null>()`
  //     — which account each seat belonged to, kept for the length of a round
  //       and surviving removeSeat. A seat may leave mid-round (leavesMidHand
  //       is true here) and what it is owed does not leave with it; once
  //       Seating.remove has run, the seat is the only record of whose account
  //       it was, so settlement would have nobody to pay and the chips would
  //       cease to exist. In a ring that is the economy losing chips outright,
  //       which is the mirror of minting them and no better. Registered in
  //       join(); pruned to the seats still present in beginRound(), because
...
describe("a chip going down at a casino table", () => {
  ...
  it("pays a seat that stood up before the coins were read", async () => {
    ...
    const before = held["u0"]! + held["u1"]! + held["u2"]!;
    ...
    const after = held["u0"]! + held["u1"]! + held["u2"]!;
...
In `games/two-up/src/adapter.ts`, replace the stub `botMove` with the real one. Roulette's is the model for its shape, but **two things must not be copied from it**, because both are wrong there too.

**A bot must pay for its own chip.** Roulette's bot calls `table.place(...)` directly, never through `stake()`, so its chip sits on the felt paid for by nobody while settlement later pays the bot as though it had staked. `BotMove.play()` is synchronous and cannot await `stake()` — but it does not need to, because a bot only ever plays at a for-fun table, whose purse and bank are both fields on the table. Add a synchronous `stakeFun(table, seatId, chips): boolean` beside `stake` (refusing when the purse is short, else `movePurse(seatId, -chips)` and `funBank += chips`), and a `refundFun` that reverses it. `play()` then stakes first, acts second, and refunds if the table refuses.

**A bot must only be offered a move it is allowed to make.** Roulette has one betting window in which every seat may bet, so its loop can take the first bot it finds. A ring cannot: only the spinner may set a centre, and the spinner may not cover their own. Offering an ineligible bot and letting the table refuse it is silent — `play()` swallows the error, the loop picks the same seat next cycle, its pile count never advances, and the bot that *could* act is never reached, so the round stalls for the whole window while something appears to be happening.

So gate selection on a `mayAct(table, seatId)` helper: casino requires `phase === "betting"`; a ring in `centre` requires `seatId === table.spinnerId`; a ring in `covering` requires a centre to exist, the seat not to be its owner, and `uncovered(...)` to be at least `MIN_CHIP`. Cap a cover at `uncovered(...)` and skip the seat when the remainder is below the smallest chip.

Otherwise as roulette: `null` unless `table.forFun` and not last call; first seated, non-waiting, eligible bot; `botBet`; a `BotMove` whose `play()` swallows only a `TableError`.

- [ ] **Step 5b: Test the wiring, not just the arithmetic**

`bot.ts`'s functions are pure and easy to test; the adapter wiring is where both bugs above lived, and a test of `botBet` alone catches neither. Add tests to `adapter.test.ts` asserting:

- **a bot pays for the chip it puts down** — seat a bot at a for-fun casino table, cross a round boundary so it is dealt in, take one `botMove` and `play()` it, then assert the chips on the cloth equal the fall in its purse and the rise in `funBank`;
- **a bot is never offered a cover it cannot make** — on a for-fun ring with a centre up, poll `botMove` twenty times and assert no offered move belongs to the spinner, and that no cover recorded belongs to them either.
...
export function rattle(closest = 0.004): number[] {
  ...
  for (let sample = 1; sample <= 2_000; sample += 1) {
    ...
    /*
     * The decay read backwards, and that inversion is the whole of it.
     *
     * `progress` describes something losing speed: its crossings crowd the
     * start and thin out towards the end. A coin settling flat does the
     * opposite — it goes over, and the knocks come faster and faster as it
     * flattens, which is the sound everybody recognises. Walking the curve
     * forwards gave a rattle that slowed down, measurably: gaps growing from
     * 0.02 to 0.124 instead of shrinking.
     *
     * `1 - at(1 - t)` turns the same curve round. It still starts at nought
     * and ends at one, so nothing else has to change.
     */
...
describe("a coin", () => {
  ...
  it("chalks a cross on the tail and never on the head", () => {
     ...
     * are struck into it before it is ever thrown, so both sit in the DOM on
     * every render and the `face` prop only chooses the angle it settles at.
     * Rendering a head and expecting no chalk anywhere in the container would
     * fail against that construction for a reason with nothing to do with
     * chalk being on the wrong side — so the assertion reaches into each face
     * rather than judging the whole coin by which way it landed.
     */
...
- The edge is a ring of 20 `.tu__coin-edge` divs, each `rotateZ(n * 18deg) rotateY(90deg) translateZ(radius)`, giving a real cylinder. That is what stops it reading as a flipping card.

  The order matters and an earlier draft of this plan had it wrong. `rotateX(n * 18deg) translateZ(radius)` sweeps the segment through the YZ plane, which traces a hoop *through* the middle of the coin rather than around its rim — it does not draw an edge at all. `rotateZ` puts the segment at its angle around the face, `rotateY(90deg)` stands it up so its normal points radially outward, and only then does `translateZ` push it out to the rim.

  Give each segment a chord a little longer than `circumference / 20` — about 8% — or twenty flat faces leave twenty hairline gaps.
...
Create `apps/web/src/twoup/twoup.css` with the coin's keyframes: `tu-flight` (the arc, using `FLIGHT`'s easing), `tu-tumble` (the rotation, `SPIN`), `tu-wobble` (the settle, `WOBBLE`).

**Every keyframe needs an off switch that actually wins.** This is where an earlier draft of this plan was wrong, and the way it was wrong is worth stating because it is invisible: a media query adds **no specificity**. An override written as

```css
@media (prefers-reduced-motion: reduce) {
  .tu__coin-spin { animation: none; }   /* (0,1,0) */
}
```

loses outright to `.tu__coin--flying .tu__coin-spin` (0,2,0), and loses worse to `.tu__coin:not(.tu__coin--flying) .tu__coin-wobble` (0,3,0) — `:not()` contributes its argument's specificity. The result is a page that looks like it respects the setting, silences the one animation whose selectors happen to tie, and runs the other two at full speed.

So the reduced-motion block must **repeat each motion rule's selector exactly**, not a shortened version of it:

```css
@media (prefers-reduced-motion: reduce) {
  .tu__coin--flying,
  .tu__coin--flying .tu__coin-spin,
  .tu__coin:not(.tu__coin--flying) .tu__coin-spin,
  .tu__coin:not(.tu__coin--flying) .tu__coin-wobble,
  .tu__coin-face {
    animation: none;
  }
  .tu__coin-face {
  ...
  /* Blank as well as edge-on: with no tumble to blur the faces together, the
     face has to stop rendering rather than sit there readable a whole flight
     before the result is due. */
  .tu__coin--flying .tu__coin-face {
    opacity: 0;
  }
}
```

- [ ] **Step 3b: Pin the off switches in a test**

A specificity loss is silent in jsdom and silent in review — it shipped past both here. `apps/web/src/game/buttons.css.test.ts` is the repo's precedent for the answer: read the stylesheet and assert on its text, for the reason that file states — *"a copy of the rules would agree with itself forever, and this fails if the rule is dropped."*

Create `apps/web/src/twoup/motion.css.test.ts`. It reads `twoup.css`, finds **every** selector that declares `animation: tu-`, and asserts each one also appears inside the `prefers-reduced-motion` block. Written that way it needs no updating when a fourth animation is added — it fails until that animation has an off switch too, which is the whole point.
...
- Produces: cues `"kip" | "coinRing" | "coinClack" | "coinRattle" | "headsUp" | "tailsUp" | "oddsUp"`; `function tossCoins(options: { flightMs: number; wobbleMs: number; height: (t: number) => number; landings: readonly number[]; rattle: readonly number[] }): () => void`; `function useTossSound(flying: boolean, flightMs: number, shape): void`; `function useCalledSound(outcome: Outcome | null, flying: boolean): void`.
...
Add `tossCoins` to `apps/web/src/game/audio.ts`, beside `spinWheel` and following its shape exactly (early return when muted or contextless, one `bus` gain, a `stop` list, and a returned teardown).

**Every curve it needs arrives as an argument.** `audio.ts` is the building's shared sound module and must not import one game's physics — which is exactly why `spinWheel` takes its `ticks` as an option rather than reaching into `roulette/spin.ts`. Follow that, and follow it for *all* of it: `landings`, `rattle`, **the height envelope and the wobble's length too**.

An earlier draft of this plan left the last two unstated, and the result was a copy of `height()` written out inside `audio.ts` (`4 * at * (1 - at)`, with a comment admitting it "echoes" the original) and a hand-mirrored `WOBBLE_MS = 420` matching the stylesheet. Two sets of numbers that have to agree is the precise failure `toss.ts` exists to prevent, and a comment saying so does not prevent it — the two drift the first time anybody tunes one.
...
export function tossCoins(options: {
  ...
  wobbleMs: number;
  /** The coin's height over its flight, nought on the felt and one at the apex — `height`. */
...
- [ ] **Step 4b: One wobble length, not two**

`WOBBLE_MS` cannot live in the stylesheet and in the scheduler both. Export it from `toss.ts`:
...
Then `Coin.tsx` hands it to CSS as a custom property (`"--wobble-ms": `${WOBBLE_MS}ms``), `twoup.css` animates `tu-wobble var(--wobble-ms, 420ms)`, and `useTossSound` passes the same constant to `tossCoins`. One number, three readers, no copy.

What it synthesises, in order:

- **kip** — a short filtered-noise sweep, bandpass rising then cut, ~120ms. Wood moving air.
**One trap in the ring's envelope, which crashed the page.** `setValueCurveAtTime` owns the whole span `[start, start + duration]`, and scheduling *any* other automation inside it throws `NotSupportedError` — which in a browser is an uncaught exception that takes the page down, not a silent miss. Damping the ring after its curve must therefore land strictly **after** the curve ends, with a real margin rather than exactly on the boundary:
...
```ts
       ...
       * Scheduling the damp at exactly `now + duration` lands on the boundary
       * and is a coin toss in itself: it threw on the second voice and not
       * the first, because floating point put one just inside the span.
       */
...
Nothing in a mocked test can catch this: every audio test in this repo replaces the context. It is only visible with real ears or a real `AudioContext`.
...
**`TwoUp.tsx` must import the theme itself** — `import "@backroom/game-two-up/theme.css";` — and not rely on `Room.tsx` having done it. Every other game's page does the same (`Roulette.tsx:22`, `Blackjack.tsx:40`, `Slots.tsx:31`, `Tips.tsx:13`), and the reason is that `/two-up/:code` is its own route: somebody following a link straight to a table never renders `Room`, so the room would arrive unpainted.
...
Create `apps/web/src/style/TwoUpMockup.tsx` beside `RouletteMockup.tsx` — importing the theme itself, as `RouletteMockup.tsx:8` does, and register it in `apps/web/src/style/Gallery.tsx`. A still life: the ring, the kip, two coins caught at the top of the arc.
