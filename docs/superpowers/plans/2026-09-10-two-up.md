...
One thing the kip test pins that is easy to miss: `closeBetting()` names the spinner from `kipAt` **without** advancing it, and `beginRound()` advances it. Advancing in both is how the kip skips every other player.
