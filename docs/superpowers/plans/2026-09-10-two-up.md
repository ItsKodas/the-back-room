...
describe("a casino round with odds in it", () => {
  ...
  it("puts every throw on the board, odds included", () => {
    ...
    table.closeBetting();
    let at = 0;
    table.boxerThrows(() => (at++ % 2 === 0 ? 0.1 : 0.9));
    table.land();
    table.read();
    throwing(table, 0.1);
...
export class Table {
  ...
  //     — the traditional school's staking. `cover` refuses more than
  //       uncovered() leaves, and refuses the spinner covering their own centre.
  //
  ...
  //
  //   closeCentre(): void
  //     — "centre" -> "covering". A spinner who never set one reopens the
  //       centre window with the felt untouched, exactly as an empty cloth
  //       reopens the betting window rather than throwing.
  //
  //   closeCovering(): void
  ...
  //   read(): void      — reads the run: unresolved goes back to "spinning"
  //                       (casino) or "kip" (school); resolved settles.
...
```ts
  ...
  read(): void {
    ...
    if (this.school === "casino") {
      ...
      if (called === null) {
        this.phase = "spinning";
        this.deadline = Date.now() + FLIGHT_MS;
        return;
      }
      this.called = called;
...
- **`pause`** switches on the phase: `betting`/`centre`/`covering` run to their deadline, `kip` expires into `boxerThrows`, `spinning` into `land`, `reading` into `read`, `settled` into `beginRound`. The key carries the throw index — `` `spinning:${table.throws.length}` `` — for the reason `GameAdapter.pause` documents.
...
```ts
    ...
    pause(table) {
      ...
      switch (table.phase) {
        ...
        case "reading":
          return { key: `reading:${at}`, ms: Math.min(left(), readMs), run: () => table.read() };
        case "settled":
...
The room is cold where the wheel's is hot: a shed, a chalked ring on boards, a bare bulb over it. `--gr-color-felt: #232a2e` and `--gr-color-night: #141618` are already promised by the listing and must match. Add the materials nothing else in the building has:
...
`twoup.css` is loaded by `Coin.tsx` or `TwoUp.tsx`. Confirm with `grep -rn "twoup.css" apps/web/src` before moving on — an orphan stylesheet is a rule that is silently dead.
