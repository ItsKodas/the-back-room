...
- Produces:
  ...
  - `multiplier(spot: Spot, roll: Roll, hand: Hand): number` — what one chip **hands over**, which for a bet that stays up is its winnings alone.
...
import { after, decided, type Hand, maxOdds, multiplier, nextPoint, sleeps } from "./resolve.js";
...
describe("the pass line", () => {
  it("wins on seven and eleven and loses to the craps numbers, on a come-out", () => {
    for (const n of [7, 11]) expect(multiplier(at("pass"), rollOf(n), comeOut)).toBe(2);
    for (const n of [2, 3, 12]) expect(multiplier(at("pass"), rollOf(n), comeOut)).toBe(0);
    for (const n of [7, 11, 2, 3, 12]) expect(after(at("pass"), rollOf(n), comeOut)).toBeNull();
  });

  it("stays put while a point is being set", () => {
    for (const n of POINTS) {
      expect(multiplier(at("pass"), rollOf(n), comeOut)).toBe(0);
      expect(after(at("pass"), rollOf(n), comeOut)).toBe("pass");
    }
  });

  it("wins on its point and loses to the seven", () => {
    expect(multiplier(at("pass"), rollOf(6), on(6))).toBe(2);
    expect(after(at("pass"), rollOf(6), on(6))).toBeNull();
    expect(multiplier(at("pass"), rollOf(7), on(6))).toBe(0);
...
describe("don't pass", () => {
  it("bars the twelve rather than winning on it", () => {
    ...
    expect(multiplier(at("dontpass"), rollOf(12), comeOut)).toBe(1);
    expect(after(at("dontpass"), rollOf(12), comeOut)).toBeNull();
  });

  it("wins on two and three, loses to seven and eleven", () => {
    for (const n of [2, 3]) expect(multiplier(at("dontpass"), rollOf(n), comeOut)).toBe(2);
    for (const n of [7, 11]) expect(multiplier(at("dontpass"), rollOf(n), comeOut)).toBe(0);
  });

  it("wins on the seven once a point is on", () => {
    expect(multiplier(at("dontpass"), rollOf(7), on(8))).toBe(2);
    expect(multiplier(at("dontpass"), rollOf(8), on(8))).toBe(0);
  });
});
...
describe("come bets travel", () => {
  it("moves to the number it rolled rather than resolving", () => {
    ...
    expect(multiplier(at("come"), rollOf(5), on(9))).toBe(0);
  ...
  it("acts as its own come-out in the box, whatever the table's point is", () => {
    expect(multiplier(at("come"), rollOf(7), on(9))).toBe(2);
    expect(multiplier(at("dontcome"), rollOf(11), on(9))).toBe(0);
    expect(multiplier(at("dontcome"), rollOf(12), on(9))).toBe(1);
  });

  it("wins on its own number once it has travelled", () => {
    expect(multiplier(at("come:5"), rollOf(5), on(9))).toBe(2);
    expect(multiplier(at("come:5"), rollOf(7), on(9))).toBe(0);
    expect(multiplier(at("dontcome:5"), rollOf(7), on(9))).toBe(2);
    expect(multiplier(at("dontcome:5"), rollOf(5), on(9))).toBe(0);
  });
});
...
describe("odds pay true, which is the whole point of them", () => {
  it("pays taken odds at the real chance of the number", () => {
    ...
    expect(multiplier(at("odds:pass"), rollOf(4), on(4))).toBe(3);
    expect(multiplier(at("odds:pass"), rollOf(5), on(5))).toBe(1 + 3 / 2);
    expect(multiplier(at("odds:pass"), rollOf(6), on(6))).toBe(1 + 6 / 5);
  });

  it("pays laid odds the other way up", () => {
    expect(multiplier(at("odds:dontpass"), rollOf(7), on(4))).toBe(1 + 1 / 2);
    expect(multiplier(at("odds:dontpass"), rollOf(7), on(5))).toBe(1 + 2 / 3);
    expect(multiplier(at("odds:dontpass"), rollOf(7), on(6))).toBe(1 + 5 / 6);
  });
  ...
  it("takes its number from the spot when it is behind a come bet", () => {
    ...
    expect(multiplier(at("odds:come:6"), rollOf(6), on(9))).toBe(1 + 6 / 5);
    expect(multiplier(at("odds:come:6"), rollOf(9), on(9))).toBe(0);
  });
  ...
  it("caps taken odds at three, four and five times the line", () => {
    ...
    for (const point of POINTS) {
      ...
      expect(odds * (multiplier(at("odds:pass"), rollOf(point), on(point)) - 1)).toBe(600);
    }
  });
  ...
  it("caps a lay at whatever would win the same", () => {
    for (const point of POINTS) {
      ...
      const win = lay * (multiplier(at("odds:dontpass"), rollOf(7), on(point)) - 1);
...
describe("the numbers stay up when they win", () => {
  it("hands over winnings only and keeps the bet on the cloth", () => {
    ...
    expect(multiplier(at("place:6"), rollOf(6), on(9))).toBe(7 / 6);
    expect(after(at("place:6"), rollOf(6), on(9))).toBe("place:6");
    expect(multiplier(at("place:4"), rollOf(4), on(9))).toBe(9 / 5);
    expect(multiplier(at("place:5"), rollOf(5), on(9))).toBe(7 / 5);
    expect(multiplier(at("big:8"), rollOf(8), on(9))).toBe(1);
    expect(after(at("big:8"), rollOf(8), on(9))).toBe("big:8");
  });

  it("is taken down by the seven", () => {
    for (const id of ["place:6", "place:4", "big:8"]) {
      expect(multiplier(at(id), rollOf(7), on(9))).toBe(0);
...
describe("the field, the hardways and the middle", () => {
  it("pays the field its three different prices", () => {
    for (const n of [3, 4, 9, 10, 11]) expect(multiplier(at("field"), rollOf(n), on(8))).toBe(2);
    expect(multiplier(at("field"), rollOf(2), on(8))).toBe(3);
    expect(multiplier(at("field"), rollOf(12), on(8))).toBe(4);
    for (const n of [5, 6, 7, 8]) expect(multiplier(at("field"), rollOf(n), on(8))).toBe(0);
  ...
  it("pays a hardway only on the pair, and stays up when it does", () => {
    expect(multiplier(at("hard:8"), pair(8), on(9))).toBe(9);
    expect(after(at("hard:8"), pair(8), on(9))).toBe("hard:8");
    expect(multiplier(at("hard:4"), pair(4), on(9))).toBe(7);
  });

  it("loses a hardway to the easy way and to the seven", () => {
    expect(multiplier(at("hard:8"), easy(8), on(9))).toBe(0);
  ...
  it("prices the one-roll propositions", () => {
    expect(multiplier(at("any7"), rollOf(7), on(9))).toBe(5);
    expect(multiplier(at("anycraps"), rollOf(3), on(9))).toBe(8);
    expect(multiplier(at("two"), rollOf(2), on(9))).toBe(31);
    expect(multiplier(at("twelve"), rollOf(12), on(9))).toBe(31);
    expect(multiplier(at("three"), rollOf(3), on(9))).toBe(16);
    expect(multiplier(at("eleven"), rollOf(11), on(9))).toBe(16);
  });
  ...
  it("splits the horn four ways and the C and E two", () => {
    ...
    expect(HORN_STEP * multiplier(at("horn"), rollOf(2), on(9))).toBe(465);
    expect(HORN_STEP * multiplier(at("horn"), rollOf(3), on(9))).toBe(240);
    expect(HORN_STEP * multiplier(at("horn"), rollOf(11), on(9))).toBe(240);
    expect(HORN_STEP * multiplier(at("horn"), rollOf(12), on(9))).toBe(465);
    expect(multiplier(at("horn"), rollOf(7), on(9))).toBe(0);
    expect(MIN_CHIP * multiplier(at("ce"), rollOf(12), on(9))).toBe(120);
    expect(MIN_CHIP * multiplier(at("ce"), rollOf(11), on(9))).toBe(240);
  });
});
...
describe("the rulebook as a whole", () => {
  ...
  it("never pays a fraction of a chip, for any chip in the tray", () => {
    ...
    for (const spot of SPOTS.values()) {
      ...
      for (const chips of CHIPS.filter((one) => one % step === 0)) {
        for (const one of OUTCOMES) {
          for (const hand of hands) {
            const back = chips * multiplier(spot, one, hand);
            expect(Number.isInteger(back), `${spot.id} ${chips} ${one} ${hand.point}`).toBe(true);
          }
        }
      }
    }
      ...
      for (const one of OUTCOMES) {
        for (const hand of hands) {
          if (after(spot, one, hand) !== null) {
            expect(multiplier(spot, one, hand), `${spot.id}`).toBe(0);
          }
        }
      }
    }
  ...
  it("never pays more than thirty-one times a chip, which is what the cap rests on", () => {
    for (const spot of SPOTS.values()) {
      for (const one of OUTCOMES) {
        for (const hand of hands) {
          expect(multiplier(spot, one, hand), `${spot.id}`).toBeLessThanOrEqual(31);
        }
      }
    }
  });
...
/** Taken odds: the real chance of the number against the seven. */
const TRUE_ODDS: Readonly<Record<number, number>> = {
  4: 2,
  10: 2,
  5: 3 / 2,
  9: 3 / 2,
...
/** Laid odds: the same fraction the other way up, because the bet is reversed. */
const LAY_ODDS: Readonly<Record<number, number>> = {
  4: 1 / 2,
  10: 1 / 2,
  5: 2 / 3,
  9: 2 / 3,
...
/** What a place bet wins per chip, keeping the chip on the cloth. */
const PLACE_ODDS: Readonly<Record<number, number>> = {
  4: 9 / 5,
  10: 9 / 5,
  5: 7 / 5,
  9: 7 / 5,
...
const HARD_ODDS: Readonly<Record<number, number>> = { 4: 7, 10: 7, 6: 9, 8: 9 };

/** The one-roll middle: what it wins on, and what it pays. */
const PROP_ODDS: Readonly<Record<string, readonly [number, number]>> = {
  any7: [7, 4],
  two: [2, 30],
  three: [3, 15],
  eleven: [11, 15],
 ...
 *
 * Zero for a bet that lost, and zero for a bet that was simply not this roll's
 * business. One is a push: the chip back and nothing on it.
 */
export function multiplier(spot: Spot, roll: Roll, hand: Hand): number {
  const t = total(roll);
  switch (spot.kind) {
    case "line":
      return line(spot, t, hand);
    case "travelled":
      return spot.dark ? (t === 7 ? 2 : 0) : t === spot.number ? 2 : 0;
    case "odds":
      return odds(spot, t, hand);
    case "place":
      return t === spot.number ? (PLACE_ODDS[spot.number as number] as number) : 0;
    case "big":
      return t === spot.number ? 1 : 0;
    case "field":
      if (t === 2) return 3;
      if (t === 12) return 4;
      return [3, 4, 9, 10, 11].includes(t) ? 2 : 0;
    case "hard":
      return isHard(roll) && t === spot.number ? (HARD_ODDS[spot.number as number] as number) : 0;
    case "prop":
 ...
 */
function line(spot: Spot, t: number, hand: Hand): number {
  ...
  if (spot.dark) {
    if (ownComeOut) {
      ...
      if (CRAPS_NUMBERS.has(t)) return t === 12 ? 1 : 2;
      return 0;
    }
    return t === 7 ? 2 : 0;
  }
 ...
 * have four come bets working and each wants its own price.
 */
function odds(spot: Spot, t: number, hand: Hand): number {
  const n = spot.number ?? hand.point;
  if (n === null) return 0;
  if (spot.dark) return t === 7 ? 1 + (LAY_ODDS[n] as number) : 0;
  return t === n ? 1 + (TRUE_ODDS[n] as number) : 0;
}

function prop(id: string, t: number): number {
  const single = PROP_ODDS[id];
  if (single !== undefined) {
    return t === single[0] ? single[1] + 1 : 0;
  }
  if (id === "anycraps") return CRAPS_NUMBERS.has(t) ? 8 : 0;
  if (id === "horn") {
    // A quarter of the chips on each of the four, so a hit pays its own odds
    // on a quarter and the other three quarters are gone.
...
export function maxOdds(point: number, line: number, dark: boolean): number {
  ...
  return Math.floor((6 * line) / (LAY_ODDS[point] as number));
}
...
export type { Hand } from "./resolve.js";
export { after, decided, maxOdds, multiplier, nextPoint, sleeps } from "./resolve.js";
```
  ...
  it("offers exactly what the bank can carry, never a chip more", () => {
    ...
    for (const spot of SPOTS.values()) {
      for (const hand of [comeOut, on(6), on(4)]) {
        ...
        if (most === 0 || most > 1_000_000) continue;
...
import { type Hand, multiplier } from "./resolve.js";
...
function most(bet: Bet, hand: Hand): number {
  ...
  for (const roll of OUTCOMES) {
    const back = bet.chips * multiplier(bet.spot, roll, hand);
...
export function owed(bets: readonly Bet[], hand: Hand): number {
  ...
  for (const roll of OUTCOMES) {
    ...
    for (const one of bets) {
      if (one.off) continue;
      here += one.chips * multiplier(one.spot, roll, hand);
    }
...
export function headroom(bank: number, bets: readonly Bet[], hand: Hand, spot: Spot): number {
  ...
  for (const roll of OUTCOMES) {
    const mult = multiplier(spot, roll, hand);
    if (mult <= 1) continue;
    let here = 0;
    for (const one of bets) {
      if (one.off) continue;
...
import { after, type Hand, multiplier } from "./resolve.js";
...
export function settle(
  ...
  for (const one of placed) {
    ...
    const back = one.chips * multiplier(spot, roll, hand);
...
const PILES = { cautious: 2, normal: 3, reckless: 5 } as const;
...
export function botBet(
  ...
  const most = PILES[skill] ?? PILES.normal;
  if (piles >= most) {
    return null;
  }
  const pick = <T>(from: readonly T[]): T => from[Math.floor(Math.random() * from.length)] as T;
  const spotId =
    piles === 0 ? pick(LINE) : skill === "reckless" && Math.random() < 0.4 ? pick(MIDDLE) : pick(NUMBERS);
