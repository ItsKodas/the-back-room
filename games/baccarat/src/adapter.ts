...
import { BACCARAT } from "./listing.js";
import { spotAt } from "./spots.js";
import { SETTLE_MS, Table, WINDOWS } from "./table.js";
...
/**
 * What the room does with a baccarat table.
 *
 * The same money as the wheel's, and for the same reasons: every chip goes
 * into the bank as it lands and nothing comes back out until the coup is
 * decided, so there is no per-hand settling to interleave. The timing is
 * simpler even than the wheel's — baccarat has no decisions in it, so nobody
 * ever has a turn — which leaves the pause below as the entire engine.
 *
 * What is genuinely different here is the shape of the cap. The wheel's
 * payouts are whole multiples of the stake; the banker line is not, because
 * the commission steps rather than slopes. That difference lives in
 * `bank.ts`'s `headroom`, and nothing in this file needs to know about it.
 */
export function baccaratAdapter(
  options: {
    random?: () => number;
    /** How long the felt is open for bets. An argument so a test can hurry it. */
    window?: number;
    /** How long a settled coup stays up to be read. */
    settleMs?: number;
    /**
     * The bank these tables pay from, when there is one.
     *
     * Without it a table takes chips off accounts and hands winnings back with
     * nothing in between, which is a shoe that mints. With it, every chip is in
     * the bank before a card is dealt and every payout comes out of it, so the
     * table can only ever hand over what somebody put there.
     */
    bank?: Bank;
  } = {},
): GameAdapter<Table> {
   ...
   *
   * Every baccarat table in the building is paid from one bank, so a cap worked
  ...
  /** Coups that have been paid, so a settled cloth stops holding the bank's chips. */
  const paidOut = new WeakSet<object>();

  /**
   * A coup handed to `settle` and still waiting its turn in the queue.
   *
   * Counted apart from the table, because the table does not wait for it: a
   * cloth swept for the next window no longer says what the last coup is owed.
   */
  const unpaid = new WeakMap<Table, { coup: object; back: number }>();

  /** What a void has closed the escrow on and not yet paid back out of the bank. */
  const refunding = new WeakMap<Table, number>();

  const backOf = (coup: ReadonlyMap<string, { back: number }>): number => {
    let back = 0;
    for (const one of coup.values()) {
      back += one.back;
    }
    return back;
  };
   ...
   *
   * The cloth's worst outcome until the coup lands, what the coup actually
  ...
  const owing = (table: Table): number => {
    ...
    } else if (table.paid !== waiting?.coup && !paidOut.has(table.paid)) {
  ...
  /**
   * What the bank holds, not counting this coup's chips.
   *
   * The figure every cap is worked out against. Chips go into the bank as they
   * land, so what it holds mid-window already includes them — and `headroom`
   * adds the stakes back itself, so handing it the inflated figure would let
   * the cloth vouch for itself.
   *
   * Nor counting what every other table paid from this bank could owe. Their
   * chips are in there too, and they are not this table's to promise. Only a
   * fact inside `serially`, where nothing else can move the bank between
   ...
   * cheaper for it: Player went down against the bank, Banker was then allowed
   * to lean on Player — the two sides cannot both come in, so between them they
   * need far less bank than either alone — and taking Player back leaves Banker
   * owed more than the bank holds. So it is refused, but only when it makes the
   * shortfall worse, so a bank drained from outside never traps anybody's chips
   * on a cloth.
   *
  ...
  const giveBack = async (
    ...
    if (off === null) {
      throw new TableError("That chip is covering another bet. It stays for this coup.");
    }
  ...
  const pay = async (
     ...
     * against the worst outcome on this exact cloth, so this cannot refuse —
     * which is exactly why it is checked. The alternative is a bank going
     * negative in silence and a shoe that has quietly started minting.
     *
     * And loudly, because a refusal now means something outside the book moved
    ...
    if (bank !== null && !(await bank.take(chips))) {
      console.error(
        `baccarat ${table.code}: the bank refused ${chips} owed to ${seat.userId ?? seat.id}`,
      );
      return;
    }
    if (seat.userId !== null) {
  ...
  return {
      ...
      await serially(table, async () => {
        switch (move.type) {
          case "place": {
             ...
             * Everybody's chips, not just this seat's: one coup settles all of
             ...
             */
            const most = headroom(await base(table), toBets(table.placed), spot.id);
            if (chips > most) {
     ...
     * synchronous, so the table can only note who left, and whether their chips
     * can come off is a cover check against the bank; and what the bank holds
     * is a question for the store, which a view cannot ask because building one
     * is synchronous.
     *
     * A for-fun table never needs this: its bank is on the table and its figure
     * is exact from the moment it exists. Which is the good half of doing it
     * this way — the number can only ever be stale where staleness costs a
     * greyed-out spot on a table nobody is sitting at yet.
     *
     * Less what the other tables on this bank could owe, so the felt greys out
     * the same spots the refusal would. Showing only; `place` asks again.
     */
    async payOut(table, deps) {
      if (table.leaving.size > 0) {
        await serially(table, async () => {
           ...
           * has gone too — and if everybody has left, no coup is coming to
    ...
    /**
     * Pays what the coup owes.
     *
     * Every stake is already in the bank, so this only ever moves winnings out.
     * The bank cannot refuse: the cap on every chip was worked out against the
     * worst outcome on this exact cloth, so whatever the shoe said, what is
     * owed now was covered before a card came out. It is checked anyway,
     * because the alternative to checking is a bank that goes negative in
     * silence.
     *
     * In the bank's queue, so that no other table reads the bank with this
     * coup's winnings paid out of it and still counted as owed, or the other
     * way round. The coup is read before joining the queue, not after: the
     * table sweeps its cloth on its own clock, and a settle that looked for the
     * result once its turn came could find nothing there to pay.
     */
    async settle(table, deps) {
      const coup = table.paid;
      if (coup === null || table.coup === null) {
        return;
      }
      if (banked(table)) {
        unpaid.set(table, { coup, back: backOf(coup) });
      }
      /*
       * A decided payout is theirs, seated or not: chips that rode belong to
       * whatever the shoe did, so somebody who stood up is owed what it dealt.
       * Who that is gets settled here, before the first await. The account
       ...
       * paying for this coup.
       */
      const payees = [...coup].map(([seatId, paid]) => ({
        seat: {
          id: seatId,
    ...
    async void(table, deps) {
      const due = table.escrow.close();
      refunding.set(
        table,
        due.reduce((total, one) => total + one.chips, 0),
      );
      await serially(table, async () => {
        try {
    ...
    /** Who came out of the coup ahead, which is not the same as who was paid. */
    winners(table) {
      if (table.paid === null) {
        return [];
      }
