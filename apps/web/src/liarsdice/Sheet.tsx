...
export function Sheet({
  ...
  className: "ld__sheet--felt" | "ld__sheet--page";
  children: ReactNode;
}) {
  ...
  return (
    <>
      <button type="button" className="ld__scrim" tabIndex={-1} aria-label={`Close ${label}`} onClick={onClose} />
      <div id={id} className={`ld__sheet ${className}`} role="dialog" aria-label={label} ref={panel} tabIndex={-1}>
        <div className="ld__sheet-head">
          <h2 className="ld__sheet-title">{heading ?? label}</h2>
        ...
        <div className="ld__sheet-body table-scroll">{children}</div>
      </div>
    </>
  );
}
